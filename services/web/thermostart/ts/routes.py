...
from .utils import decrypt_request, encrypt_response, Source, get_patched_firmware_by_hw_version
...
def api():
    ...
    if device.cal_synced == False:
        xml = xml + '<CAL>'

        # tm_wday in thermostat starts at 0 being Sunday, webinterface says 0 is Monday
        tm_wday = [1, 2, 3, 4, 5, 6, 0] # "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"

        std_week = ''
        for block in device.standard_week:
            std_week = std_week + 's{:01d}{:02d}{:02d}{:03d}{:01d}'.format(tm_wday[block['start'][0]], 
                                                                block['start'][1], 
                                                                block['start'][2], 
        ...
        for block in device.exceptions:
            ...
            start = datetime(block['start'][0], block['start'][1] + 1, block['start'][2], 0, 0, tzinfo=timezone(timedelta(seconds=-time.timezone)))
            end = datetime(block['end'][0], block['end'][1] + 1, block['end'][2], 0, 0, tzinfo=timezone(timedelta(seconds=-time.timezone)))
            ...
            exc_week = exc_week + 'x{:08X}{:08X}{:03d}{:01d}X'.format(start, end, device.predefined_temperatures[block['temperature']], 0)
    ...
    if int(tsreq['pv'][0]) != device.measured_temperature:
        ...
        db.session.commit()
        emit('measured_temperature', 
             {'measured_temperature': int(tsreq['pv'][0])}, 
             namespace='/', 
             to = hardware_id)

    if not device.outside_temperature_timestamp or datetime.fromtimestamp(device.outside_temperature_timestamp) + timedelta(seconds = 3600) < datetime.utcnow():
        location = Location.query.filter_by(id=device.location_id).one()
        if location is None:
            return Response(response='no location', status=400)

        querystring = {"location" : f"{location.latitude}, {location.longitude}", 
        ...
        emit('outside_temperature', {'location': location.city, 
                                     'outside_temperature': outside_temperature, 
                                     'outside_temperature_icon': None},
                                     namespace='/', to = hardware_id)
    ...
    # is there a change from the webinterface?
        ...
        if device.ui_source == 'pause_button':
            ...
            xml = xml + f'<SD>{device.sd}</SD>'
        elif device.ui_source == 'direct_temperature_setter_up' or device.ui_source == 'direct_temperature_setter_down':
            xml = xml + f'<PAUSE>0</PAUSE>'
            xml = xml + f'<INIT><SRC>{device.source}</SRC></INIT>'
            xml = xml + f'<SVSET>{device.set_temperature}</SVSET>'
        ...
        if tssrc == Source.MANUAL.value and 'csv' in tsreq and int(tsreq['csv'][0]) != device.set_temperature:
            device.source = Source.MANUAL.value
            db.session.commit()
            emit('set_temperature', {'set_temperature': int(tsreq['csv'][0])}, namespace='/', to = hardware_id)
