...
from thermostart.ts.utils import Source, Display, StatusLed
...
class Device(UserMixin, db.Model):
    ...
    def get_default_predefined_labels():
        return {'anti_freeze': 'Anti freeze', 
                'comfort': 'Comfort', 
                'home': 'Home', 
                'not_home': 'Not home', 
                'pause': 'Pause'}
    ...
    def get_default_standard_week():
        return [{'start': [0, 6, 30], 'temperature': 'home'},
    ...
    predefined_temperatures = db.Column(MutableDict.as_mutable(JSON), default=get_default_predefined_temperatures)
    predefined_labels = db.Column(MutableDict.as_mutable(JSON), default=get_default_predefined_labels)
    standard_week = db.Column(MutableList.as_mutable(JSON), default=get_default_standard_week)
    dhw_programs = db.Column(MutableList.as_mutable(JSON), default=get_default_dhw_programs)
